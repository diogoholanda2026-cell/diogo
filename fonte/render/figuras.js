...
export class Crowd {
  constructor(kind = 'pessoa', max = 200) {
    this.mesh = new THREE.InstancedMesh(figureGeos()[kind], figureMaterial(), max); this.mesh.count = 0; this.mesh.castShadow = false; this.mesh.receiveShadow = true; this.mesh.frustumCulled = false;
    this.max = max; this.walkers = []; this._m = new THREE.Matrix4(); this._q = new THREE.Quaternion(); this._p = new THREE.Vector3(); this._s = new THREE.Vector3(1, 1, 1); this._c = new THREE.Color(); this._e = new THREE.Euler();
  }
  add(path, o = {}) { // path: [[x,y,z],...], o: {speed, loop, color, idle}
    ...
    const i = this.walkers.length; this.walkers.push(w); this.mesh.count = this.walkers.length;
    this._c.setRGB(w.col[0], w.col[1], w.col[2]); this.mesh.setColorAt(i, this._c); this.mesh.instanceColor.needsUpdate = true;
    return w;
  }
  clear() { this.walkers.length = 0; this.mesh.count = 0; }
  update(dt, t) {
    ...
    for (let i = 0; i < W.length; i++) {
      ...
      this._e.set(0, ang, 0); this._q.setFromEuler(this._e); this.mesh.setMatrixAt(i, this._m.compose(this._p, this._q, this._s));
    }
