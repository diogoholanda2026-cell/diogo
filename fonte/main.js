...
import { carregar, gravar, lerConfig, persistir } from './core/salvar.js';
...
async function iniciar() {
  ...
  const engine = new Engine(canvas, { quality: qs.get('q') || 'alta', fixedPR: +qs.get('pr') || 0, fpsCap: cfg.fps || 60 });
  ...
  let forest = null;
  const env = new Environment(engine); engine.onQuality = (q) => { env.setShadowSize(q.shadow); env.key.shadow.radius = q.radius; forest?.setShadows(q.treeShadow); };
  env.setShadowSize(engine.q.shadow); env.key.shadow.radius = engine.q.radius; if (cfg.luz) env.setMode(cfg.luz);
  ...
  // save
  let S = await carregar(); if (!S || S.v !== VERSAO_SAVE || qs.has('novo')) S = novoEstado(Date.now());
  ...
  C.vistaFoto = (anim) => { const v = VISTA_FOTO; rig.pitchFix = v.pitch; const o = { x: v.x, z: v.z, dist: v.dist, yaw: v.yaw, fov: v.fov, roll: v.roll }; C._naFoto = true; if (anim) rig.flyTo(o, 1600); else { rig.target.set(o.x, 0, o.z); rig.dist = o.dist; rig.yaw = o.yaw; rig.fov = o.fov; rig.roll = o.roll; rig.apply(); } };
  ...
  const loop = (t) => {
    ...
    ocioso = moveu || rig.g ? 0 : ocioso + dt; engine.idle = ocioso > 8 && !obras.ativo;
