...
// J.planoMeta() → {acao, alvo, item, n, meta, texto, fim?} ou null. acao: 'aprovar' | 'coletar' | 'iniciar' |
...
import { PROJETOS, PROJ, MODULOS, LIMITE_CAP, POP_NIVEL, POOL_NIVEL, CUSTO_NIVEL, TEMPO_NIVEL, SERVICO_NIVEL, BEM_NIVEL } from '../data/obras.js';
...
const COFRE_H = 4, PASSO_OFF = 5 * 60e3, PASSOS_MAX = 288, RAMPA_MS = 48 * 3600e3;
...
const REPASSE = [24, 0.0025], MEDICAO_ETAPA = 0.7, MEDICAO_MODULO = 0.7, DISP = { pedido: 2, modulo: 4, etapa: 8, marco: 30 };
...
export const MIGRACOES = {
  1: (S, agora) => { // v1 → v2: escolhas viram efeitos, teto de 3 fichas, epílogo em duas etapas, itens de capítulos adiante
    const esc = {}; for (const [n, id] of Object.entries(S.capEscolhas || {})) esc[n] = id && !EFEITOS[id] ? 'legado:' + id : id ?? null;
    S.capEscolhas = esc; S.bonus = bonusZero();
    const m = Math.max(0, num(S.mutirao) | 0); if (m > FICHAS_MAX) { S.disposicao = Math.min(100, num(S.disposicao) + 100 * (m - FICHAS_MAX)); S.mutirao = FICHAS_MAX; }
    const r = S.etapas?.['reflorestar.e1']; if (r && r.estado && r.estado !== 'prancha') S.etapas['reflorestar.e0'] = { estado: 'feita', entregue: {} };
    ...
    for (const p of Object.values(S.predios || {})) { for (const f of p.fila || []) if (adiante(f?.item)) leg.add(f.item); for (const k of p.prontos || []) if (adiante(k)) leg.add(k); }
    for (const p of S.pedidos || []) for (const k of Object.keys(p?.itens || {})) if (adiante(k)) leg.add(k);
...
export function normalizar(S, agora = Date.now()) {
  ...
  for (const [k, v] of Object.entries(S.itens || {})) if (!ITENS[k]) guarda('itens', k, v);
  const itemOk = (k) => !!ITENS[k] && ITENS[k].tipo !== 'especial';
  // prédios
  O.predios = {};
  for (const [id, P] of Object.entries(PREDIOS)) {
    const b = B.predios[id], s = objeto(S.predios?.[id]) ? S.predios[id] : null; const o = { ...(s || {}), ...b, ok: s ? !!s.ok : b.ok };
    if (P.tipo === 'usina') { o.nSlots = clamp(num(s?.nSlots, 3) | 0, 3, USINA_MAX); o.slots = Array.from({ length: o.nSlots }, (_, i) => { const x = s?.slots?.[i]; return x && ITENS[x.item]?.tipo === 'bruto' ? { item: x.item, ini: num(x.ini, O.t), fim: num(x.fim, O.t) } : null; }); }
    else if (P.tipo === 'oficina') {
      o.nFila = clamp(num(s?.nFila, 3) | 0, 3, FILA_SELO);
      o.fila = []; for (const f of s?.fila || []) { if (!itemOk(f?.item)) { guarda('fila', id + ':' + (f?.item || '?'), f); continue; } const g = { item: f.item, ini: num(f.ini), fim: num(f.fim) }; if (f.pend) { g.pend = true; g.desde = num(f.desde, O.t); g.ini = g.fim = 0; } o.fila.push(g); }
      o.prontos = (s?.prontos || []).filter((k) => { if (itemOk(k)) return true; guarda('prontos', id + ':' + k, k); return false; });
      if (o.fila.length > o.nFila) o.nFila = Math.min(FILA_SELO, o.fila.length);
    }
    O.predios[id] = o;
  }
  for (const [id, v] of Object.entries(S.predios || {})) if (!PREDIOS[id]) guarda('predios', id, v);
  // etapas: só as que existem; entregas acima do pedido voltam para o almoxarifado
  O.etapas = {};
  for (const [key, st] of Object.entries(S.etapas || {})) {
    const [pid, eid] = key.split('.'); const e = PROJ[pid]?.etapas.find((x) => x.id === eid); if (!e || !objeto(st)) { guarda('etapas', key, st); continue; }
    const estado = ['prancha', 'obra', 'pronta', 'feita'].includes(st.estado) ? st.estado : 'prancha'; const n = { ...st, estado, entregue: {} };
    const req = { ...e.itens, ...(e.licencas || {}) };
    for (const [k, q] of Object.entries(st.entregue || {})) { const v = Math.max(0, Math.round(num(q))); if (!ITENS[k]) { guarda('entregue', key + ':' + k, v); continue; } const u = Math.min(v, req[k] || 0); if (u) n.entregue[k] = u; if (v > u) O.itens[k] += v - u; }
  ...
  for (const [f, N] of Object.entries(N_MODULOS)) {
    ...
    O.modulos[f] = Array.from({ length: N }, (_, i) => {
      ...
      if (m.pedido && (!objeto(m.pedido.itens) || Object.keys(m.pedido.itens).some((k) => !ITENS[k]))) delete out.pedido;
      return out;
    });
    if (arr.length > N) guarda('modulos', f, arr.slice(N));
  }
  for (const [f, v] of Object.entries(S.modulos || {})) if (!N_MODULOS[f]) guarda('modulos', f, v);
  // pedidos, topógrafo, bem-estar temporário, escolhas
  O.pedidos = (Array.isArray(S.pedidos) ? S.pedidos : []).filter(objeto).map((p) => (p.itens && Object.keys(p.itens).some((k) => !ITENS[k]) ? { id: p.id, espera: O.t, itens: null } : { ...p, espera: num(p.espera, O.t) }));
  O.topografo = objeto(S.topografo) && TOPOGRAFO[S.topografo.k] ? { k: S.topografo.k, ini: num(S.topografo.ini, O.t), fim: num(S.topografo.fim, O.t) } : null;
  ...
  O.legado = Array.isArray(S.legado) ? S.legado.filter((k) => ITENS[k]) : [];
...
export class Jogo {
  ...
  itensEtapa(p, e) { const key = p.id + '.' + e.id; const f = this.ef.itensEtapa[key]; const out = {}; for (const [k, n] of Object.entries(e.itens)) out[k] = f ? Math.max(1, Math.round(n * f)) : n; for (const [k, n] of Object.entries(this.ef.maisItens[key] || {})) out[k] = (out[k] || 0) + n; for (const [k, n] of Object.entries(e.licencas || {})) out[k] = n; return out; }
  ...
  bemEstar() { let b = 35 + this.ef.bem + this._bemTemp() - this.populacao() / 250; for (const p of PROJETOS) for (const e of p.etapas) if (e.bem && this.feita(p.id + '.' + e.id)) b += e.bem; return Math.round(clamp(b, 0, 100)); }
  bemInfo() {
    ...
    fontes.sort((a, b) => b.v - a.v); return { total: this.bem, base: 35, fontes, pressao: Math.round(this.pop / 250) };
  }
  ...
  _efeitos() { // soma dos efeitos das escolhas do Conselho (e do bônus antigo, se houver)
    const S = this.S; const E = { bem: 0, almox: 0, repasse: 0, energia: 0, usina: 0, oficina: 0, xp: 0, fila: 0, oficinas: {}, itens: {}, custoEtapa: {}, tempoEtapa: {}, itensEtapa: {}, maisItens: {}, requer: {}, semRequer: {}, modulo: {} };
    for (const k of ['usina', 'oficina', 'almox', 'repasse', 'bem', 'xp']) E[k] += num(S.bonus?.[k]);
    for (const id of Object.values(S.capEscolhas || {})) {
      const ef = EFEITOS[id]; if (!ef) continue;
  ...
  aprovarEtapa(key) {
    ...
    const med = Math.round((st.pago ?? this.custoEtapa(p, e)) * MEDICAO_ETAPA); delete st.pago; S.creditos += med; // medição: a Holding paga pela etapa medida
  ...
  requisitosModulo(f, i) { // consulta pura
    ...
    const custo = CUSTO_NIVEL[n]; const servicos = []; for (const [lv, ks] of Object.entries(SERVICO_NIVEL)) if (+lv <= n) servicos.push(...ks);
    const popDepois = this.pop + Math.round(MODULOS[f].pop * ((POP_NIVEL[n] ?? 1) - POP_NIVEL[m.nivel]));
    const servOk = servicos.every((k) => this.serv[k] >= popDepois); const bemMin = BEM_NIVEL[n] || 0; const bemOk = this.bem >= bemMin;
  ...
  aprovarModulo(f, i) {
    ...
    m.nivel = m.obra.para; m.obra = null; delete m.pedido; const med = Math.round(CUSTO_NIVEL[m.nivel] * MEDICAO_MODULO); this.S.creditos += med;
  ...
  _pedidos(agora) {
    const S = this.S; if (S.cap < 2) return; const alvo = S.cap === 2 ? 3 : S.cap === 3 ? 4 : 6; const P = S.pedidos;
  ...
  _repor(i, ms) { if (this.S.cap < 2) this.S.pedidos.splice(i, 1); else this.S.pedidos[i] = { id: this.S.seq++, espera: this.agora + ms, itens: null }; }
  // capítulos
  metaFeita(m) {
  ...
  _planoModulo(f, alvoNivel, vistos) {
    ...
    for (const i of cands) {
      ...
      const falta = {}; for (const [k, n] of Object.entries(r.itens)) if ((S.itens[k] || 0) < n) falta[k] = n - (S.itens[k] || 0);
      ...
      const pl = this._planoItens(falta); if (pl.acao !== 'aguardar') return pl; espera = this._espera(espera, pl); break;
    }
  ...
  _emProducao(k) { // quanto de k está pronto para coletar e quanto está a caminho
    const S = this.S; let pronto = 0, curso = 0, fim = Infinity, onde = null;
    ...
    const I = ITENS[k]; if (I.oficina) { const o = S.predios[I.oficina]; if (o.ok) { for (const x of o.prontos) if (x === k) { pronto++; onde = I.oficina; } for (const f of o.fila) if (f.item === k) { curso++; if (f.fim) fim = Math.min(fim, f.fim); } } }
    return { pronto, curso, fim, onde };
  }
  _planoItem(k, n, prof) {
    ...
    if (Object.entries(I.req).every(([r, q]) => (S.itens[r] || 0) >= q)) {
      if (o.fila.length < this.vagasFila(I.oficina)) return this._plano('produzir', { predio: I.oficina }, `Produzir ${faltam} ${nomeIt(k)} ${noPredio(I.oficina)}`, { item: k, n: faltam });
