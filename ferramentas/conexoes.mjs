...
import { normals } from '../fonte/render/geom.js';
...
for (const [k, f] of Object.entries(F)) { const pts = []; const p = f.prof; for (const m of f.mods) { const nor = normals(m.path, false); m.path.forEach(([x, z], i) => { for (let t = 0; t <= 1.0001; t += 0.25) { const o = p.o0 + (p.o1 - p.o0) * t; pts.push([x + nor[i][0] * o, z + nor[i][1] * o]); } }); } pegada[k] = pts; }
...
const circ = {
  ...
  gorilas: [A.gorilas.c, A.gorilas.rx + 0.15, A.gorilas.rz + 0.15, 0], acelerador: [A.acelerador.c, A.acelerador.rx + 0.7, A.acelerador.rz + 0.6, 0],
...
// contornos precisos: lóbulos da Ciências (mesma função blob do modelo), casas da vila, cerca dos gorilas, CRD
function blob(cx, cz, rx, rz, rot, seed, n = 150) { const o = []; const c = Math.cos(rot), s = Math.sin(rot); const p1 = hash(seed, 1, 3) * 6, p2 = hash(seed, 2, 3) * 6; for (let i = 0; i < n; i++) { const a = (i / n) * Math.PI * 2; const k = 1 + 0.1 * Math.sin(2 * a + p1) + 0.07 * Math.sin(3 * a + p2); const x = Math.cos(a) * rx * k, z = Math.sin(a) * rz * k; o.push([cx + x * c - z * s, cz + x * s + z * c]); } return o; }
const cc = A.ciencias; const lobos = { loboA: blob(cc.c[0] + 2.6, cc.c[1] - 0.9, 4.3 + 0.2, 2.7 + 0.2, cc.rot - 0.12, 4), loboB: blob(cc.c[0] - 3.2, cc.c[1] - 1.6, 2.7 + 0.2, 1.8 + 0.2, cc.rot + 0.3, 9) };
const v = A.vila; const casas = [[-1.9, 0.2], [-1.15, -0.25], [-0.4, 0.25], [0.4, -0.2], [1.15, 0.3], [1.9, -0.1]].map(([x, z]) => { const r = v.rot, co = Math.cos(r), si = Math.sin(r); return [[-0.62, -0.42], [0.62, -0.42], [0.62, 0.42], [-0.62, 0.42]].map(([u, w]) => { const lx = x + u, lz = z + w; return [v.c[0] + lx * co + lz * si, v.c[1] - lx * si + lz * co]; }); });
const g = A.gorilas; const cerca = []; for (let i = 0; i < 64; i++) { const a = (i / 64) * Math.PI * 2; cerca.push([g.c[0] + Math.cos(a) * (g.rx + 0.1), g.c[1] + Math.sin(a) * (g.rz + 0.1)]); }
const cr = A.biblio.crd; const crdPoly = [[-4.6, -2.1], [5.8, -2.1], [5.8, 2.8], [3.2, 2.8], [3.2, 2.1], [-4.6, 2.1]].map(([lx, lz]) => { const co = Math.cos(cr.rot), si = Math.sin(cr.rot); return [cr.c[0] + lx * co + lz * si, cr.c[1] - lx * si + lz * co]; });
const polys = { ...lobos, cerca, crd: crdPoly, ...Object.fromEntries(casas.map((p, i) => ['casa' + i, p])) };
...
{ const ns = Object.keys(polys); for (let i = 0; i < ns.length; i++) for (let j = i + 1; j < ns.length; j++) { if (ns[i] === 'loboA' && ns[j] === 'loboB') continue; const d = polyPoly(polys[ns[i]], polys[ns[j]]); if (d < 0.15 && !(ns[i].startsWith('casa') && ns[j].startsWith('casa'))) report(`${ns[i]} x ${ns[j]}: ${d.toFixed(2)}`); } }
for (const [n, P] of Object.entries(polys)) { let m = 1e9; for (const [x, z] of P) m = Math.min(m, escD(x, z) - 0.42 - 0.05); if (m < 0.3) report(`escarpa x ${n}: folga ${m.toFixed(2)}`); }
...
// 4) modelos x modelos
const cn = Object.keys(circ); for (let i = 0; i < cn.length; i++) for (let j = i + 1; j < cn.length; j++) { const [c1, a1, b1] = circ[cn[i]], [c2, a2, b2] = circ[cn[j]]; const d = Math.hypot(c1[0] - c2[0], c1[1] - c2[1]) - Math.max(a1, b1) - Math.max(a2, b2); if (d < 0) report(`modelo ${cn[i]} x ${cn[j]}: ${d.toFixed(2)} (aprox. por círculos)`); }
