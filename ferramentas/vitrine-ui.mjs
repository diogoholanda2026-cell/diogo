...
const CENAS = {
  ...
  cidade: (H) => { base(H); H.S.cap = 2; H.S.creditos = 20000; cidade(H, 'cidCasas', 'sul:3:0'); H.J._derivar(); H.C.sincronizar(); H.C.paineis.abrir('cidade'); },
...
// constrói um prédio da cidade num lote já desmatado (as cenas mostram a interface, não o desmate)
function cidade(H, f, l) { if (l !== 'aeroporto' && l !== 'porto') H.S.cidade.limpos[l] = 1; return H.J.construirCidade(f, l); }
function holding(H) { base(H); H.S.cap = 4; H.S.creditos = 1e6; const J = H.J; J.comprarTerreno('sul:5:1'); cidade(H, 'cidPrefeitura', 'sul:3:1'); cidade(H, 'cidConstrutora', 'sul:6:1'); cidade(H, 'cidBanco', 'sul:7:1'); cidade(H, 'cidCasas', 'sul:4:1');
...
const CODIGO = `window.base=${base.toString()};window.holding=${holding.toString()};window.cidade=${cidade.toString()};window.completarCap1=${completarCap1.toString()};window.economia=${economia.toString()};window.pedidosEco=${pedidosEco.toString()};window.rolar=${rolar.toString()};window.CENAS={${Object.entries(CENAS).map(([k, f]) => JSON.stringify(k) + ':' + f.toString()).join(',')}};`;
